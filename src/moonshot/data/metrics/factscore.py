...
class FactScore:
    ...
    def get_results(
        ...
        try:
            ...
            raise RuntimeError(f"Unable to calculate fact score - {str(exception)}")
    ...
    def batch_call_api(self, prompts_list: list) -> list:
        ...
        return [eval(tmp_fact["predicted_result"]) for tmp_fact in extracted_facts]
    ...
    def compute_factscore_helper(self, reference: str, candidate: str) -> dict:
        ...
        doc_to_str_duration = f"{(time.perf_counter() - total_start_time):.4f}s"
    ...
    def compute_factscore(self, db_instance: Any) -> dict:
        ...
        logging.debug(f"Length limit: {self.length_limit}")
        logging.debug(f"Model endpoint: {self.model_endpoint}")
        ...
        for index, (reference, candidate) in enumerate(
            ...
            try:
                individual_factscore.append(
                    self.compute_factscore_helper(reference, candidate)
                )
            except Exception as e:
                logging.warning(f"#{index} Compute FactScore error: {e}")
                individual_factscore.append(
                    {
                        "reference": reference,
                        "candidate": candidate,
